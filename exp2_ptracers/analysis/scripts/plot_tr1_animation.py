"""
plot_tr1_animation.py
Animate PTRACER01 (microplastics) on the LLC270 Arctic cap grid.
Run from the exp2_ptracers experiment directory.

Output: analysis/figures/postrun/tr1_animation.gif
"""
...
timesteps = [timestep_from_file(f) for f in tr1_files]
...
sc = ax.scatter([], [], s=2, c=[], cmap='plasma',
...
cb = fig.colorbar(plt.cm.ScalarMappable(norm=Normalize(0, tr1_max), cmap='plasma'),
                  ax=ax, fraction=0.03, pad=0.04, shrink=0.6)
cb.set_label('TR1 concentration', color='white', fontsize=8)
...
def update(frame):
    ...
    mask = tr1 > 1e-6
    if mask.any():
        sc.set_offsets(np.column_stack([XC[mask], YC[mask]]))
        sc.set_array(tr1[mask])
    else:
        sc.set_offsets(np.empty((0, 2)))
    days = timesteps[frame] * DT / 86400
    title.set_text(f'Arctic microplastics transport  |  day {days:.0f}  (step {timesteps[frame]})')
    return sc, title
